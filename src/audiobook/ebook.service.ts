...
export class EbookService {
  ...
  async listarEbooks(page: number, itemsPerPage: any, search?: string, categoryNames?: string[]) {
    ...
    let categoryFilter = 'AND EXISTS (SELECT 1 FROM RfdNV3uAM_term_relationships AS R2 INNER JOIN RfdNV3uAM_terms AS C2 ON R2.term_taxonomy_id = C2.term_id WHERE R2.object_id = P.ID AND C2.name = "AUDIOLIVROS")';
    ...
    const query = `
      ...
      (SELECT C2.name FROM RfdNV3uAM_term_relationships AS R2 INNER JOIN RfdNV3uAM_terms AS C2 ON R2.term_taxonomy_id = C2.term_id WHERE R2.object_id = P.ID AND C2.name = 'AUDIOLIVROS') AS categoria,
      ...
      result.map(
        async (ebook: {
          ...
          return {
            ...
            downloads: downloads,
            categoria: categoriaPrincipal ? categoriaPrincipal.categoria : 'AUDIOLIVROS',
  ...
  async obterEbookPorId(id: any) {
    ...
    const cacheKey = `audiobook_${ebookId}`;
    const cachedData = await this.cacheManager.get(cacheKey);
    if (cachedData) {
      return cachedData;
    }
    ...
    const query = `
      ...
      AND EXISTS (
          ...
          WHERE R2.object_id = P.ID AND C2.name = 'AUDIOLIVROS'
      )
      GROUP BY P.ID`;
    ...
    if (!result || result.length === 0) {
      throw new NotFoundException(`Audiobook com ID ${ebookId} não encontrado.`);
    }
